...
### 每个月，品项数，采购金额
 select a.orderMonth, sum(itemCount) itemCount, sum(totalAmt) totalAmt
   from po_head a 
group by a.orderMonth
order by a.orderMonth; 

### 按供应商看，品项数，采购金额
insert into po_vender_stats (vendorCode, totalItem, totalAmt)
 select a.vendorCode, sum(itemCount) itemCount, sum(totalAmt) totalAmt
   from po_head a 
group by a.vendorCode; 

## 更新供应商名字
update po_vender_stats a 
inner join (
select a.vendorCode, a.vendorName 
  from po_head a 
group by a.vendorCode, a.vendorName
) b on a.vendorCode = b.vendorCode 
set a.vendorName = b.vendorName; 

select b.*
  from po_vender_stats b 
order by b.totalAmt desc, b.vendorCode; 
...
insert into po_vender_mat (vendorCode, matCode, totalAmt, totalQty)
 select b.vendorCode, a.material, sum(a.totalAmt10) totalAmt, sum(a.receivedQuantity) totalQty
  from po_item a
inner join po_head b on a.orderNum = b.orderNum
group by b.vendorCode, a.material; 
...
###  供应商 和 项目 的对应关系
insert into po_vendor_project (vendorCode, project, totalAmt)
 select a.vendorCode, b.project, sum(a.totalAmt) totalAmt
   from  po_vender_mat a 
inner join project_mat b on a.matCode = b.partNum 
group by a.vendorCode , b.project; 
										

ALTER TABLE po_vender_mat ADD INDEX po_vender_mat_matCode (matCode); 	
ALTER TABLE po_vender_mat ADD INDEX po_vender_mat_vendorCode (vendorCode); 	

ALTER TABLE project_mat ADD INDEX project_mat_partNum (partNum); 	
								
update po_vendor_project a 
inner join po_vender_stats b on a.vendorCode = b.vendorCode 
set a.vendorName = b.vendorName; 
...
select *, right(orderNum,1) plantCode from po_head; 

update po_head  
  set plantInd = right(orderNum,1) ; 
  
update po_head a 
inner join plant_info b on a.plantInd = b.plantInd
  set a.plantName = b.plantName; 

select * from po_head where orderNum = 'BS17333'; 

select orderNum, batch from po_item 
where orderNum = 'BS17333'
group by orderNum, batch; 
